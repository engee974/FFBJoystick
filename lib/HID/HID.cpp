...
int HID_::getInterface(uint8_t* interfaceCount)
{
	*interfaceCount += 1; // uses 1
	HIDDescriptor hidInterface = {
		...
		D_ENDPOINT(USB_ENDPOINT_OUT(pluggedEndpoint+1), USB_ENDPOINT_TYPE_INTERRUPT, 0x40, 0x01)
	};
...
int HID_::SendReport(uint8_t id, const void* data, int len)
{
	...
	p[0] = id;
	memcpy(&p[1], data, len);
	return USBD_Send(USB_ENDPOINT_IN(pluggedEndpoint), p, len+1);
}

int HID_::RecvReport(void* data)
{
        return USBD_Recv(USB_ENDPOINT_OUT(pluggedEndpoint+1), data, 64);
}

int HID_::Available(void)
{
	return USBD_Available(USB_ENDPOINT_OUT(pluggedEndpoint+1));
}
